...
def calibration():
    ...
    if BaseHeight != '':
        ...
        label(20+500, 40, "Base-Height : ", font=('Times New Roman bold', 16))
        label(130+20+500, 41, "      ")
        label(130+20+500, 41, BaseHeight, font=('Times New Roman', 16),
              fcolor="#ffcc66", bcolor="#264653")
        label(130+20+60+500, 41, "cm", font=('Times New Roman bold', 16))
        return BaseHeight
...
def newFile():
    ...
    if filename_ != "" or location != "":
        if shipment != "":
            ...
            filename = folderPath + filename_ + ".xlsx"
            wb = openpyxl.Workbook()
            ws = wb.active
            ws.append(fieldnames)
            wb.save(str(filename))
            wb.save(str(filename_ + ".xlsx"))
            # file.save(str(fullLocation))
            # file.save(str(filename_))
...
def selectExcel():
    ...
    label(730, 60+15, "                                  ")
    label(730, 60+15, text=filename[:-5])
    label(800, 190+15-60, "         ")
    label(800, 190+15-60, shipment)
    displayData()
...
def uploadData():
    # file.save(fullLocation)
    filteredImageLocation = imageNameLocation.replace("\\", "/")
    filteredFileLocation = fullLocation.replace("\\", "/")
    # payload = {'name': str(filename_)}
    # files = [
    #     ('file', (str(filename), open(str(fullLocation), 'rb'),
    #      'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')),
    #     ('image', (str(imageNameOnly), open(str(filteredImageLocation), 'rb'), 'image/jpeg'))
    # ]

    payload={}
    files = [
        ('file', (str(os.path.basename(filteredFileLocation)), open(str(filteredFileLocation),
         'rb'), 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')),
        ('image', (str(os.path.basename(filteredImageLocation)),
         open(str(filteredImageLocation), 'rb'), 'image/jpeg'))
    ]
#     files=[
...
# ]
    ...
    print(response.text)
    print("Excel Location : "+str(filteredFileLocation))
    print("Excel Name : "+str(os.path.basename(filteredFileLocation)))
    print("Photo Location : "+str(filteredImageLocation))
    print("Photo Name : "+str(os.path.basename(filteredImageLocation)))
    # if(response):
    #     return True
    # else:
    #     return False
...
def addData():
    ...
    #                             message="Enter Shipment Details")
        ...
        file = openpyxl.load_workbook(str(fullLocation))
        sheet = file.active

        # workbook = xlsxwriter.Workbook(str(fullLocation))
        # worksheet = workbook.add_worksheet("data")

        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # timestamp = datetime.now().strftime("%m/%d/%Y %H:%M")
        # timestamp = datetime(2023, 1, 31, 21, 38, 36)

        # reader = pd.read_excel(str(fullLocation))
        # reader.append({
        #     # "S.N": count,
        #     "Timestamp": timestamp,
        ...
        info = [
            ...
            int(newWeight.get()),
            volume,
            volumetricWeightFirst,
            volumetricWeightSecond
        ]
        sheet.append(info)
        
        file.save(str(fullLocation))
        file.close()
        # file.save(str(filename_ + ".xlsx"))
        # file.save(str(filename_))
        # wb.save('example.xlsx')
        # print(info)
        # worksheet.write_row(sheet.max_row, 0, info)
        # workbook.close()

        # info.to_excel(reader, sheet_name='Sheet1', index=False)
...
def takeSnapshot():
    ...
    global filename_
    global shipment
    global imageNameOnly
    global imageNameLocation

    agency = filename_[:4]
    img = Image.fromarray(img1)
    imageFolderPath = "Snapshots\\"+filename_+"\\"
    if not os.path.exists(imageFolderPath):
        os.makedirs(imageFolderPath)
    imageNameOnly = str(filename_+"-"+shipment +
                        "-"+newSerial.get()) + ".jpg"
    imageName = imageFolderPath + imageNameOnly
    imageNameLocation = os.path.abspath(imageName)
    img.save(imageName)
...
for port in ports:
    ...
    if "USB-SERIAL" in port.description:
        if __name__ == '__main__':
            try:
                ser = serial.Serial(str(port[0]), baudRate, timeout=1)
                # ser.flush()
...
def displayData():
    ...
    if fullLocation is not None:
        mylist.delete(0, 'end')
        wb = openpyxl.load_workbook(fullLocation)
        ws = wb.active
        # last_row = ws.max_row
        # last_row_values = [
        #     ws.cell(row=last_row, column=col).value for col in range(1, ws.max_column+1)]
        # for row in last_row_values:
        #     print(row)
        for row in ws.iter_rows(min_row=2, values_only=True):
            print(row)

        # if(last_row_values[0] != "Timestamp"):
            mylist.insert(
                END, "--------------------------------------------------------------------------------------------")
            # mylist.insert(END, row["Timestamp"] + "        |       " + row["BoxNumber"] + "       |       " +
            #               row["Length (cm)"] + "       |       " + row["Breadth (cm)"] + "       |       " + row["Height (cm)"] + "       |       " + row["Weight (kg)"])
            # mylist.insert(END,str(row[1]) + "       |       " + str(row[2]) +
            #               "       |       " + str(row[3]) + "       |       " + str(row[4]) + "       |       " + str(row[5]))
            mylist.insert(END, str(row[0]) + "        |       " + str(row[1]) + "       |       " + str(row[2]) +
                          "       |       " + str(row[3]) + "       |       " + str(row[4]) + "       |       " + str(row[5]))
...
def internetStatus():
    status = "Online" if connect() else "Offline"
    label(650, 320+25+35-50, "System Status : ")
    label(650+120, 320+25+35-50, status,
          bcolor=("#00FF00" if connect() else "#FF0000"))
...
label(650+130+30, 390+25+35, BreadthAdditional)
# label(880, 45, "S.N |")
label(880+10, 45, "Data / Time | BoxNumber | Length | Breadth | Height | Weight")
# label(880+10, 45, "Data/Time |")
...
button(650, 260, "Select an Excel file", cmd=selectExcel)
